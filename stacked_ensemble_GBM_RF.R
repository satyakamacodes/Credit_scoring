...
# Generating a combination of GBM and RF using Stacked Ensemble

#GBM
my_gbm <- h2o.gbm(x = c(1:57,59:61),
                  y = 58,
...
h2o.confusionMatrix(my_gbm, d2)

#RF

my_rf <- h2o.randomForest(x = c(1:57,59:61),
                          y = 58,
                          training_frame = train,
                          validation_frame = test,
                          nfolds = 5,
...
h2o.confusionMatrix(my_rf, d2)

# Train a stacked ensemble using the GBM and RF above
ensemble <- h2o.stackedEnsemble(x = c(1:57,59:61),
                                y = 58,
                                training_frame = train,
...
perf <- h2o.performance(ensemble, newdata = d2)

h2o.auc(ensemble)

perf_gbm_test <- h2o.performance(my_gbm, newdata = d2)
perf_rf_test <- h2o.performance(my_rf, newdata = d2)
...
pred <- h2o.predict(ensemble, newdata = d2)



h2o.confusionMatrix(ensemble, d2)
