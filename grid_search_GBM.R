...
# GBM hyperparamters
gbm_parameters <- list(learn_rate = seq(0.01, 0.1, 0.01),
...
gbm_gridperformance <- h2o.getGrid(grid_id = "gbm_grid", 
                                   sort_by = "auc", 
                                   decreasing = TRUE)
print(gbm_gridperformance)


#Learning rate around 0.08 gives better AUC


gbm_params_new <- list(learn_rate = seq(0.07, 0.09, 0.01),  
                       max_depth = seq(2, 10, 1),
                       sample_rate = seq(0.9, 1.0, 0.05),  
                       col_sample_rate = seq(0.1, 1.0, 0.1))

search_criteria_new <- list(strategy = "RandomDiscrete", 
...
gbm_grid <- h2o.grid(algorithm = "gbm", 
                     ...
                     ntrees = 100,
                     seed = 1,
                     hyper_params = gbm_params_new,
                     search_criteria = search_criteria_new)

gbm_gridperf_new <- h2o.getGrid(grid_id = "gbm_grid", 
                            sort_by = "auc", 
                            decreasing = TRUE)
print(gbm_gridperf_new)
...
h2o.confusionMatrix(best_gbm, d2)
